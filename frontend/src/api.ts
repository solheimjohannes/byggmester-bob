...
export function fetchUpcomingPlans(userId: string): Promise<Event[]> {
  return get<Event[]>(
    `/api/plans/upcoming?userId=${encodeURIComponent(userId)}`,
  );
}

export function fetchCreatedEvents(userId: string): Promise<Event[]> {
  return get<Event[]>(
    `/api/events/created?userId=${encodeURIComponent(userId)}`,
  );
}
...
export function fetchFriendsEvents(
  userId: string,
  limit = 10,
): Promise<Event[]> {
  return get<Event[]>(
...
export function fetchPublicEvents(
  { q, city, limit = 50 }: FetchPublicEventsParams = {},
): Promise<Event[]> {
